...
{
    /* Lazy to update, delete, sort, insert for Mysql data
     *
     * @author: Gary
     * @date: 2013/1/1
     *
    ...
    function resort($where = "", $sequ = "sequ", $tbname = "", $pk = "", $asc = "ASC")
    {
        ...
        $sql = sprintf("SELECT " . add_field_quotes($pk) . " FROM %s %s ORDER BY `%s` %s;", $tbname, $where, $sequ, $asc);
    ...
    function renew($post_arr = "", $upload = "", $sdir = "", $s_size = "", $limit = 2, $upload_file = "", $is_image = false, $MODE = '')
    {
        ...
        if (is_array($upload) && count($upload) > 0)
        {
            ...
            foreach ($upload as $k => $v)
            {
                ...
                if (isset($v['name']) && !empty($v['tmp_name']))
                {
                    ...
                    $sFileName = $prefix[0] . '_' . ($post_arr['custom_prefix'] != '' ? $post_arr['custom_prefix'] . '_' : '') . date("U") . $sExtension;
                    ...
                    if (move_uploaded_file($oFile, $sFilePath) && round(filesize($sFilePath) / 1048576, 2) < $limit)
                    {
                        ...
                        foreach ($s_size as $x => $z)
                        {
                            ...
                            $post_arr[$post_arr['image_key']] = $sFileName;
                        }
    ...
    // function move_sequ($sequid, $move, $c = '', $table = "", $obj_f = "sequ")
    // {//改變順序
