...
class Photo extends Superobj
{
    ...
    function get_all($target = 0, $parent = 0)
    {
        ...
        $wheres = " AND a.`parent` = b.`id` AND a.`parent` = " . $parent;

        $this->list_this = "SELECT a.* FROM " . $this->tbname . " a, `" . $this->tbname2 . "` b WHERE 1 AND a.`target` = " . $target . $wheres . " ORDER BY a.`sequ` ASC, a.`dates` DESC";
